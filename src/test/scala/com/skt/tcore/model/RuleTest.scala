...
class RuleTest extends FunSuite {
  ...
  test("MetricStateRule filterStr") {
    assert (
      MetricStateRule("r1", "server1", "cpu", 50, ">").condition
  ...
  test("MetricRollupRule filterStr") {
    assert (
      MetricRollupRule("r1", "server1", "cpu", "mean", 50, ">").condition
  ...
  test("LogRule filterStr") {
    assert(
      LogRule("r1", "server1", "ERROR", "=").condition
        == "log = 'ERROR'"
    )
    assert(
      LogRule("r1", "server1", "ERROR", "!=").condition
    ...
    assert(
      LogRule("r1", "server1", "ERROR", "like").condition
        == "log like '%ERROR%'"
    )
  }
}
